...
class MethodPrinter : public MatchFinder::MatchCallback {
   ...
   std::optional<std::string> translateType(QualType type) {
      ...
      if (const auto* pointerType = type->getAs<PointerType>()) {
         ...
         // else if (asString == "float* ") {
         //    return "mlir::util::RefType::get(context,mlir::FloatType::getF32(context))";
         // }
...
int main(int argc, const char** argv) {
   ...
   llvm::raw_string_ostream hStream(hContent); // XXX.h
   llvm::raw_string_ostream cppStream(cppContent); // XXX.cpp

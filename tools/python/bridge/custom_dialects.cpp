...
MlirType mlirDBVecTypeGet(MlirContext context, int size) {
   return wrap(mlir::db::DenseVectorType::get(unwrap(context), size));
}
MlirTypeID mlirDBVecTypeGetTypeID() {
   return wrap(mlir::db::DenseVectorType::getTypeID());
}
bool mlirTypeIsADBVecType(MlirType type) {
   return llvm::isa<mlir::db::DenseVectorType>(unwrap(type));
}

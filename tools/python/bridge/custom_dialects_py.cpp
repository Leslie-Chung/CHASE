...
PYBIND11_MODULE(mlir_lingodb, m) {
   ...
   mlir::python::adaptors::mlir_type_subclass(dbModule, "StringType", mlirTypeIsADBStringType, mlirDBStringTypeGetTypeID)
      .def_classmethod(
         ...
         py::arg("cls"), py::arg("context"));
   mlir::python::adaptors::mlir_type_subclass(dbModule, "VecType", mlirTypeIsADBVecType, mlirDBVecTypeGetTypeID)
      .def_classmethod(
         "get",
         [](py::object cls, MlirContext context, size_t size) {
            return cls(mlirDBVecTypeGet(context, size));
         },
         py::arg("cls"), py::arg("context"), py::arg("size"));
   //----------------------------------------------------------------------------------------------------------------------
   // RelAlg Dialect
   //----------------------------------------------------------------------------------------------------------------------

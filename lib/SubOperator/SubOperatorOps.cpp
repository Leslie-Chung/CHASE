...
ParseResult parseDistanceSpace(OpAsmParser& parser, mlir::IntegerAttr& distanceSpaceAttr) { // support vector
   std::vector<mlir::Attribute> mapping;
   ::llvm::StringRef attrStr;
   auto loc = parser.getCurrentLocation();
   if (parser.parseOptionalKeyword(&attrStr, {"L2", "IP"})) {
      return parser.emitError(
         loc, "expected keyword containing one of the following enum values for attribute 'distanceSpace' [desc,asc]");
   }
   if (!attrStr.empty()) {
      if (attrStr == "L2")
         distanceSpaceAttr = mlir::IntegerAttr::get(parser.getBuilder().getI8Type(), 1);
      else
         distanceSpaceAttr = mlir::IntegerAttr::get(parser.getBuilder().getI8Type(), 2);
   }
   return success();
}
void printDistanceSpace(OpAsmPrinter& p, mlir::Operation* op, mlir::IntegerAttr distanceSpaceAttr) { // support vector
   int type = distanceSpaceAttr.getInt();
   switch (type) {
      case 1: p << "L2"; break;
      case 2: p << "IP"; break;
      default: p << "L2"; break;
...
ParseResult parseStateColumnMapping(OpAsmParser& parser, DictionaryAttr& attr) { // support vector
...
void subop::LookupOp::print(OpAsmPrinter& p) {
   ...
   p << " " << op.getStream() << op.getState() << " ";
   if (!op.getState().getType().isa<mlir::subop::ExternalHNSWIndexType>()) {
      printCustRefArr(p, op, op.getKeys());
   } else {
      // colname dim distancespace range_constant k_constant
      printCustRef(p, op, op.getKeys()[0].dyn_cast<mlir::tuples::ColumnRefAttr>());
      p << "dim: " << op.getKeys()[1].dyn_cast<mlir::IntegerAttr>().getInt() << ",";
      p << "distance_space: " << op.getKeys()[2].dyn_cast<mlir::IntegerAttr>().getInt() << ",";
      p << "range: " << op.getKeys()[3].dyn_cast<mlir::FloatAttr>().getValueAsDouble() << ",";
      p << "k: " << op.getKeys()[4].dyn_cast<mlir::IntegerAttr>().getInt() << ",";
   }
   p << " : " << op.getState().getType() << " ";
   printCustDef(p, op, op.getRef());
   if (!op.getEqFn().empty()) {
...
// std::vector<std::string> subop::ANNLookupOp::getReadMembers() {
//    std::vector<std::string> res;
//    for (auto x : getState().getType().cast<mlir::subop::State>().getMembers().getNames()) {
//       res.push_back(x.cast<mlir::StringAttr>().str());
//    }
...
void subop::ANNScanOp::updateStateType(mlir::subop::SubOpStateUsageTransformer& transformer, mlir::Value state, mlir::Type newType) {
   if (state == getList() && newType != state.getType()) {
      auto newRefType = transformer.getNewRefType(this->getOperation(), getElem().getColumn().type);
      setElemAttr(transformer.createReplacementColumn(getElemAttr(), newRefType));
   }
}
void subop::ANNScanOp::replaceColumns(mlir::subop::SubOpStateUsageTransformer& transformer, mlir::tuples::Column* oldColumn, mlir::tuples::Column* newColumn) {
   assert(false && "should not happen");
}
void subop::ANNSetPartitionOp::updateStateType(mlir::subop::SubOpStateUsageTransformer& transformer, mlir::Value state, mlir::Type newType) {
}
void subop::ANNSetPartitionOp::replaceColumns(mlir::subop::SubOpStateUsageTransformer& transformer, mlir::tuples::Column* oldColumn, mlir::tuples::Column* newColumn) {
   if (&getValueRef().getColumn() == oldColumn) {
      setValueRefAttr(transformer.getColumnManager().createRef(newColumn));
   }
}
...
// void subop::ANNLookupOp::updateStateType(mlir::subop::SubOpStateUsageTransformer& transformer, mlir::Value state, mlir::Type newType) {
//    if (state == getState() && newType != state.getType()) {
//       auto newRefType = transformer.getNewRefType(this->getOperation(), getRef().getColumn().type);
//       setRefAttr(transformer.createReplacementColumn(getRefAttr(), newRefType));
//    }
// }
// void subop::ANNLookupOp::replaceColumns(mlir::subop::SubOpStateUsageTransformer& transformer, mlir::tuples::Column* oldColumn, mlir::tuples::Column* newColumn) {
//    assert(false && "should not happen");
// }

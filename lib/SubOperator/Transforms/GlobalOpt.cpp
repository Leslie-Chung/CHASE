      ...
      llvm::TypeSwitch<mlir::Operation*>(op)
         ...
         .Case([&](mlir::subop::MapOp mapOp) {
            ...
            for (auto computed : llvm::zip(mapOp.getComputedCols(), returnOp.getResults())) {
               auto* col = &(std::get<0>(computed).cast<mlir::tuples::ColumnDefAttr>().getColumn()); // computed col
               auto* op = std::get<1>(computed).getDefiningOp(); // return result
         ...
         // .Case([&](mlir::subop::ANNOp annOp) { // support vector
         //    // annOp->walk([&]() )
         //    annOps.push_back(annOp);
         //    // auto returnOp = mlir::cast<mlir::tuples::ReturnOp>(annOp.getPredicates().front().getTerminator());
         //    // for (auto computed : llvm::zip(mapOp.getComputedCols(), returnOp.getResults())) {
         //    //    auto* col = &(std::get<0>(computed).cast<mlir::tuples::ColumnDefAttr>().getColumn()); // computed col
         //    //    auto* op = std::get<1>(computed).getDefiningOp(); // return result
         //    //    auto fp = fingerprint(op, columnFingerPrints);
         ...
         //    //    }
         //    // }
         //    for (auto* user : annOp->getUsers()) {
         //       handleScanUserRec(user, columnFingerPrints, materializeOps, filterOps, annOps);
         //    }
         // })
   ...
   void runOnOperation() override {
      ...
      for (auto t : externalOpByTableName) { 
         ...
         for (auto m : firstJson["mapping"].get<nlohmann::json::object_t>()) { 
            columnToFirstMember.insert({m.second.get<std::string>(), m.first}); // category: category$0
         }
         for (size_t i = 1; i < t.second.size(); i++) { 
            ...
            for (auto m : otherJson["mapping"].get<nlohmann::json::object_t>()) {
               otherMemberToFirstMember.insert({m.first, columnToFirstMember.at(m.second.get<std::string>())}); // e.g. category$2: category$0
            }

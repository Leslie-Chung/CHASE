               ...
               if (auto* defOp = c.getDefiningOp()) {
                  if (auto betweenOp = mlir::dyn_cast_or_null<mlir::db::BetweenOp>(defOp)) {
                     ...
                     p = ::llvm::TypeSwitch<mlir::Type, int>(t)
                            ...
                            .Case<::mlir::db::CharType, ::mlir::db::TimestampType, ::mlir::db::IntervalType, ::mlir::FloatType>([&](mlir::Type t) { return 2; })
                            .Case<::mlir::db::StringType>([&](::mlir::db::StringType t) { return 10; })
                            .Case<::mlir::db::DenseVectorType>([&](::mlir::db::DenseVectorType t) { return t.getSize() * 2; })
                            .Default([](::mlir::Type) { return 100; });
                  }
...
static mlir::Value translateINLJ(mlir::Value left, mlir::Value right, mlir::ArrayAttr nullsEqual, mlir::ArrayAttr hashLeft, mlir::ArrayAttr hashRight, mlir::relalg::ColumnSet columns, mlir::ConversionPatternRewriter& rewriter, mlir::Location loc, std::function<mlir::Value(mlir::Value, mlir::ConversionPatternRewriter& rewriter)> fn) {
   ...
   for (auto namedAttr : rightScan.getMapping()) {
      ...
      if (valueColumns.contains(&attrDef.getColumn())) {
         keyColNames.push_back((rewriter.getStringAttr(memberName))); 
         keyColNames.push_back(mlir::TypeAttr::get(attrDef.getColumn().type));
         mapping.push_back(rewriter.getNamedAttr(memberName, attrDef));
      }
   }
   externalIndexDescription += "} }";
...
static mlir::Value translateANNForJoin(mlir::Value left, mlir::Value right, mlir::ArrayAttr hashLeft, mlir::ArrayAttr hashRight, mlir::relalg::ColumnSet columns, mlir::ConversionPatternRewriter& rewriter, mlir::Location loc, std::function<mlir::Value(mlir::Value, mlir::ConversionPatternRewriter& rewriter)> fn) {
   // hashRight -> tablename indexname colname dim distancespace range_constant k_constant computedCol
   // hashLeft -> colname dim distancespace range_constant k_constant
   auto rightScan = mlir::cast<mlir::subop::ScanOp>(right.getDefiningOp());
   auto* ctxt = rewriter.getContext();
   auto& colManager = rewriter.getContext()->getLoadedDialect<mlir::tuples::TupleStreamDialect>()->getColumnManager();
   std::string tableName = mlir::cast<mlir::StringAttr>(hashRight[0]).str();

   mlir::ArrayAttr keyANNValue = rewriter.getArrayAttr({hashRight[2].cast<mlir::tuples::ColumnRefAttr>()});
   auto keyColumns = mlir::relalg::ColumnSet::fromArrayAttr(keyANNValue);
   auto valueColumns = columns;
   valueColumns.remove(keyColumns);
   ...
   std::string indexName = mlir::cast<mlir::StringAttr>(hashRight[1]).str();
   int dim = mlir::cast<mlir::IntegerAttr>(hashRight[3]).getInt();
   uint8_t distanceSpace = mlir::cast<mlir::IntegerAttr>(hashRight[4]).getInt();

   // Create description for external index get operation
   std::string externalIndexDescription = R"({"type": "hnsw", "index": ")" + indexName + R"(", "relation": ")" + tableName +
      R"(", "dim": )" + std::to_string(dim) + R"(, "distance_space": )" + std::to_string(distanceSpace) + R"(, "mapping": { )";
   ...
   auto externalHNSWIndexType = mlir::subop::ExternalHNSWIndexType::get(rewriter.getContext(), keyStateMembers, valueStateMembers);
   mlir::Value externalHNSWIndex = rewriter.create<mlir::subop::GetExternalOp>(loc, externalHNSWIndexType, externalIndexDescription);
   // Erase table scan
   rewriter.eraseOp(rightScan->getOperand(0).getDefiningOp()); // subop.get_external
   rewriter.eraseOp(rightScan); // subop.scan

   auto entryRefType = mlir::subop::ExternalHNSWIndexEntryRefType::get(rewriter.getContext(), externalHNSWIndexType);
   auto entryRefListType = mlir::subop::ListType::get(rewriter.getContext(), entryRefType); 
   auto [listDef, listRef] = createColumn(entryRefListType, "lookup", "list");
   auto [entryDef, entryRef] = createColumn(entryRefType, "lookup", "entryref");

   auto afterLookup = rewriter.create<mlir::subop::LookupOp>(loc, mlir::tuples::TupleStreamType::get(rewriter.getContext()), left, externalHNSWIndex, hashLeft, listDef);
   ...
   nestedMapOp.getRegion().push_back(b);
   {
      mlir::OpBuilder::InsertionGuard guard(rewriter);
      rewriter.setInsertionPointToStart(b);
      mlir::tuples::ColumnDefAttr computedCol;
      if (hashRight.size() > 7) {
         computedCol = hashRight[7].cast<mlir::tuples::ColumnDefAttr>();
      } else {
         auto [computedDef, computedRef] = createColumn(rewriter.getF32Type(), "ann", "distance");
         computedCol = computedDef;
      }
      auto scan = rewriter.create<mlir::subop::ANNScanOp>(loc, list, entryDef, computedCol);
      auto gathered = rewriter.create<mlir::subop::GatherOp>(loc, scan, entryRef, rewriter.getDictionaryAttr(mapping));
      auto combined = rewriter.create<mlir::subop::CombineTupleOp>(loc, gathered, tuple);
...
static mlir::Value translateNL(mlir::Value left, mlir::Value right, bool useHash, bool useIndexNestedLoop, bool useANNIndexNestedLoop, mlir::ArrayAttr nullsEqual, mlir::ArrayAttr hashLeft, mlir::ArrayAttr hashRight, mlir::relalg::ColumnSet columns, mlir::ConversionPatternRewriter& rewriter, mlir::Location loc, std::function<mlir::Value(mlir::Value, mlir::ConversionPatternRewriter& rewriter)> fn) {
   if (useANNIndexNestedLoop) {
      return translateANNForJoin(left, right, hashLeft, hashRight, columns, rewriter, loc, fn);
   }
   if (useHash) {
...
class CrossProductLowering : public OpConversionPattern<mlir::relalg::CrossProductOp> {
   ...
   LogicalResult matchAndRewrite(mlir::relalg::CrossProductOp crossProductOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      bool useANNIndexNestedLoop = crossProductOp->hasAttr("useANNIndexNestedLoop");

      if (!useANNIndexNestedLoop) {
         rewriter.replaceOp(crossProductOp, translateNL(adaptor.getRight(), adaptor.getLeft(), false, false, false, mlir::ArrayAttr(), mlir::ArrayAttr(), mlir::ArrayAttr(), getRequired(mlir::cast<Operator>(crossProductOp.getLeft().getDefiningOp())), rewriter, loc, [](mlir::Value v, mlir::ConversionPatternRewriter& rewriter) -> mlir::Value {
                               return v;
                            }));
      } else {
         auto rightHash = crossProductOp->getAttrOfType<mlir::ArrayAttr>("rightHash");
         auto leftHash = crossProductOp->getAttrOfType<mlir::ArrayAttr>("leftHash");
         rewriter.replaceOp(crossProductOp, translateNL(adaptor.getRight(), adaptor.getLeft(), false, false, true, mlir::ArrayAttr(), rightHash, leftHash, getRequired(mlir::cast<Operator>(crossProductOp.getLeft().getDefiningOp())), rewriter, loc, [](mlir::Value v, mlir::ConversionPatternRewriter& rewriter) -> mlir::Value {
                               return v;
                            }));
      }
      return success();
...
class InnerJoinNLLowering : public OpConversionPattern<mlir::relalg::InnerJoinOp> {
   ...
   LogicalResult matchAndRewrite(mlir::relalg::InnerJoinOp innerJoinOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      bool useANNIndexNestedLoop = innerJoinOp->hasAttr("useANNIndexNestedLoop");
      ...
      rewriter.replaceOp(innerJoinOp, translateNL(adaptor.getRight(), adaptor.getLeft(), useHash, useIndexNestedLoop, useANNIndexNestedLoop, nullsEqual, rightHash, leftHash, getRequired(mlir::cast<Operator>(innerJoinOp.getLeft().getDefiningOp())), rewriter, loc, [loc, &innerJoinOp](mlir::Value v, mlir::ConversionPatternRewriter& rewriter) -> mlir::Value {
...
class SemiJoinLowering : public OpConversionPattern<mlir::relalg::SemiJoinOp> {
   ...
   LogicalResult matchAndRewrite(mlir::relalg::SemiJoinOp semiJoinOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      // bool useANNIndexNestedLoop = semiJoinOp->hasAttr("useANNIndexNestedLoop");
      bool useANNIndexNestedLoop = false;
      ...
      if (!reverse) {
         rewriter.replaceOp(semiJoinOp, translateNL(adaptor.getLeft(), adaptor.getRight(), useHash, useIndexNestedLoop, useANNIndexNestedLoop, nullsEqual, leftHash, rightHash, getRequired(mlir::cast<Operator>(semiJoinOp.getRight().getDefiningOp())), rewriter, loc, [loc, &semiJoinOp](mlir::Value v, mlir::ConversionPatternRewriter& rewriter) -> mlir::Value {
...
class MarkJoinLowering : public OpConversionPattern<mlir::relalg::MarkJoinOp> {
   ...
   LogicalResult matchAndRewrite(mlir::relalg::MarkJoinOp markJoinOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      // bool useANNIndexNestedLoop = markJoinOp->hasAttr("useANNIndexNestedLoop");
      bool useANNIndexNestedLoop = false;
      ...
      if (!reverse) {
         rewriter.replaceOp(markJoinOp, translateNL(adaptor.getLeft(), adaptor.getRight(), useHash, useIndexNestedLoop, useANNIndexNestedLoop, nullsEqual, leftHash, rightHash, getRequired(mlir::cast<Operator>(markJoinOp.getRight().getDefiningOp())), rewriter, loc, [loc, &markJoinOp](mlir::Value v, mlir::ConversionPatternRewriter& rewriter) -> mlir::Value {
...
class AntiSemiJoinLowering : public OpConversionPattern<mlir::relalg::AntiSemiJoinOp> {
   ...
   LogicalResult matchAndRewrite(mlir::relalg::AntiSemiJoinOp antiSemiJoinOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      // bool useANNIndexNestedLoop = antiSemiJoinOp->hasAttr("useANNIndexNestedLoop");
      bool useANNIndexNestedLoop = false;
      ...
      if (!reverse) {
         rewriter.replaceOp(antiSemiJoinOp, translateNL(adaptor.getLeft(), adaptor.getRight(), useHash, useIndexNestedLoop, useANNIndexNestedLoop, nullsEqual, leftHash, rightHash, getRequired(mlir::cast<Operator>(antiSemiJoinOp.getRight().getDefiningOp())), rewriter, loc, [loc, &antiSemiJoinOp](mlir::Value v, mlir::ConversionPatternRewriter& rewriter) -> mlir::Value {
...
static std::pair<mlir::Value, mlir::tuples::ColumnRefAttr> translateANNForNotJoin(mlir::Value stream, mlir::relalg::ColumnSet columns, mlir::tuples::ColumnDefAttr computedCol,
                                                                                  mlir::DenseF32ArrayAttr queryAttr, mlir::IntegerAttr distanceSpace, std::string& tableName, std::string& indexName,
                                                                                  mlir::ConversionPatternRewriter& rewriter, mlir::Location loc, mlir::ArrayAttr& parameters) {
   auto tableScan = mlir::cast<mlir::subop::ScanOp>(stream.getDefiningOp());
   auto* ctxt = rewriter.getContext();
   auto valueColumns = columns;
   auto& colManager = rewriter.getContext()->getLoadedDialect<mlir::tuples::TupleStreamDialect>()->getColumnManager();
   bool first = true;
   std::vector<Attribute> keyColNames, keyColTypes, valColNames, valColTypes; 
   std::vector<NamedAttribute> mapping; // membername -> column，例如category$0 -> @products::@category

   // Create description for external index get operation
   std::string externalIndexDescription = R"({"type": "hnsw", "index": ")" + indexName + R"(", "relation": ")" + tableName +
      R"(", "dim": )" + std::to_string(queryAttr.size()) + R"(, "distance_space": )" + std::to_string(distanceSpace.getInt()) + R"(, "mapping": { )";
   for (auto namedAttr : tableScan.getMapping()) {
      auto identifier = namedAttr.getName();
      auto attr = namedAttr.getValue();
      auto attrDef = attr.dyn_cast_or_null<mlir::tuples::ColumnDefAttr>();
      if (!first) {
      ...
      if (valueColumns.contains(&attrDef.getColumn())) {
         valColNames.push_back((rewriter.getStringAttr(memberName)));
         valColTypes.push_back(mlir::TypeAttr::get(attrDef.getColumn().type));
         mapping.push_back(rewriter.getNamedAttr(memberName, attrDef));
      }
   }
   externalIndexDescription += "} }";
   ...
   // Erase table scan
   rewriter.eraseOp(tableScan->getOperand(0).getDefiningOp()); // subop.get_external
   rewriter.eraseOp(tableScan); // subop.scan
   auto entryRefType = mlir::subop::ExternalHNSWIndexEntryRefType::get(rewriter.getContext(), externalHNSWIndexType);
   auto entryRefListType = mlir::subop::ListType::get(rewriter.getContext(), entryRefType);
   ...
   auto scan = rewriter.create<mlir::subop::ANNLookupAndScanOp>(loc, externalHNSWIndex, listDef, entryDef, computedCol, queryAttr, parameters);
   scan->setAttr("sequential", rewriter.getUnitAttr());
   auto gathered = rewriter.create<mlir::subop::GatherOp>(loc, scan, entryRef, rewriter.getDictionaryAttr(mapping));

   return {gathered.getRes(), listRef};
}

class ANNRangeSelectionLowering : public OpConversionPattern<mlir::relalg::ANNRangeSelectionOp> { // support vector
   public:
   using OpConversionPattern<mlir::relalg::ANNRangeSelectionOp>::OpConversionPattern;

   LogicalResult matchAndRewrite(mlir::relalg::ANNRangeSelectionOp annRangeSelectionOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      auto loc = annRangeSelectionOp->getLoc();
      mlir::IntegerAttr k = rewriter.getI32IntegerAttr(-1);
      if (annRangeSelectionOp->hasAttr("k")) {
         k = annRangeSelectionOp->getAttr("k").cast<mlir::IntegerAttr>();
      }
      mlir::FloatAttr range = annRangeSelectionOp.getRangeAttr();
      auto annDef = annRangeSelectionOp.getComputedCol();

      std::string indexName = annRangeSelectionOp->getAttr("ann_index").cast<mlir::StringAttr>().str();
      std::string tableName = annRangeSelectionOp->getAttr("table_identifier").cast<mlir::StringAttr>().str();
      mlir::relalg::ColumnSet requiredColumns = getRequired(annRangeSelectionOp);
      requiredColumns.insert(annRangeSelectionOp.getOrderAttr().getColumnPtr().get());
      mlir::ArrayAttr parameters = rewriter.getArrayAttr({k, range});
      rewriter.replaceOp(annRangeSelectionOp, translateANNForNotJoin(adaptor.getRel(), requiredColumns, annDef, annRangeSelectionOp.getQueryAttr(), annRangeSelectionOp.getDistanceSpaceAttr(), tableName, indexName, rewriter, loc, parameters).first);

      return success();
   }
};

mlir::Operation* existMaterializeOp(mlir::Operation* op) {
   // op->dump();
   ...
   if (op->getOperands().size() == 0) { 
      if (auto getExternalOp = mlir::dyn_cast_or_null<mlir::subop::GetExternalOp>(op)) { 
         if (getExternalOp.getRes().getType().isa<mlir::subop::TableType>()) {
            return getExternalOp.getOperation();
         }
      }
      return nullptr;
   }
   for (auto operand : op->getOperands()) {
      result = existMaterializeOp(operand.getDefiningOp());
      if (result == nullptr)
         return nullptr;
   }
   return result;
...
class ANNTopKLowering : public OpConversionPattern<mlir::relalg::ANNTopKOp> { // support vector
   public:
   using OpConversionPattern<mlir::relalg::ANNTopKOp>::OpConversionPattern;
   LogicalResult matchAndRewrite(mlir::relalg::ANNTopKOp annTopKOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      // annTopKOp.getRel().dump(); // "relalg.map" !!!
      // adaptor.getRel().dump(); // "subop.map" !!!


      mlir::subop::MapOp subOpMapOp = adaptor.getRel().getDefiningOp<mlir::subop::MapOp>();
      mlir::Operation* op = existMaterializeOp(subOpMapOp.getOperation());

      Operator current = annTopKOp;

      while (current) {
         current = current.getChildren()[0];
         if (mlir::dyn_cast_or_null<mlir::relalg::BaseTableOp>(current.getOperation())) {
            break;
         }
         if (current && std::vector<mlir::Operation*>(current->getUsers().begin(), current->getUsers().end()).size() > 1) {
            current = {};
         }
      }
      mlir::relalg::ColumnSet requiredColumns = getRequired(current);

      if (op) {
         auto loc = annTopKOp->getLoc();
         mlir::subop::GetExternalOp getExternalOp = mlir::dyn_cast<mlir::subop::GetExternalOp>(op);
         mlir::subop::ScanOp scanOp = mlir::dyn_cast<mlir::subop::ScanOp>(*getExternalOp->getUsers().begin());
         mlir::tuples::ColumnRefAttr listRef;
         {
            mlir::OpBuilder::InsertionGuard guard(rewriter);
            rewriter.setInsertionPointAfter(scanOp);

            mlir::IntegerAttr k = annTopKOp.getKAttr();
            mlir::FloatAttr range = rewriter.getF32FloatAttr(86);
            auto annDef = annTopKOp.getComputedCol();
            std::string indexName = annTopKOp->getAttr("ann_index").cast<mlir::StringAttr>().str();
            std::string tableName = annTopKOp->getAttr("table_identifier").cast<mlir::StringAttr>().str();

            mlir::ArrayAttr parameters = rewriter.getArrayAttr({k, range});
            requiredColumns.insert(annTopKOp.getOrderAttr().getColumnPtr().get());
            auto [res, resListRef] = translateANNForNotJoin(scanOp.getRes(), requiredColumns, annDef, annTopKOp.getQueryAttr(), annTopKOp.getDistanceSpaceAttr(), tableName, indexName, rewriter, loc, parameters);
            rewriter.replaceAllUsesWith(scanOp.getRes(), res);
            listRef = resListRef;
         }

         rewriter.create<mlir::subop::ANNIncreaseFilteredKOp>(loc, subOpMapOp.getStream(), listRef); 
         rewriter.replaceOp(annTopKOp, subOpMapOp.getStream());
         rewriter.eraseOp(subOpMapOp);
         // rewriter.replaceOp(annTopKOp, subOpMapOp);
...
class WindowLowering : public OpConversionPattern<mlir::relalg::WindowOp> {
   ...
   void performWindowOp(mlir::relalg::WindowOp windowOp, mlir::Value inputStream, ConversionPatternRewriter& rewriter, std::function<mlir::Value(ConversionPatternRewriter&, mlir::Value, mlir::DictionaryAttr, mlir::Location)> evaluate, mlir::tuples::ColumnRefAttr listRef, bool needPartition) const {
      ...
      if (windowOp.getPartitionBy().empty()) {
         ...
         if (needPartition) {
            rewriter.create<mlir::subop::ANNSetPartitionOp>(loc, afterLookup, listRef, colManager.createRef(referenceDefAttr.getName()));
         }
         Block* initialValueBlock = new Block;
         {
            mlir::OpBuilder::InsertionGuard guard(rewriter);
   ...
   mlir::tuples::ColumnDefAttr getListDef(mlir::Operation* op) const {
      if (auto lookupOp = mlir::dyn_cast_or_null<mlir::subop::LookupOp>(op)) {
         if (lookupOp.getState().getType().isa<mlir::subop::ExternalHNSWIndexType>())
            return lookupOp.getRef();
         else
            return {};
      }
      if (auto lookupAndScanOp = mlir::dyn_cast_or_null<mlir::subop::ANNLookupAndScanOp>(op)) {
         if (lookupAndScanOp.getState().getType().isa<mlir::subop::ExternalHNSWIndexType>())
            return lookupAndScanOp.getRef();
         else
            return {};
      }
      for (auto operand : op->getOperands()) {
         if (auto listRef = getListDef(operand.getDefiningOp())) {
            return listRef;
         }
      }
      return {};
   ...
   LogicalResult matchAndRewrite(mlir::relalg::WindowOp windowOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      mlir::tuples::ColumnRefAttr listRef;
      if (windowOp->hasAttr("FilteredK")) {
         auto listDef = getListDef(adaptor.getRel().getDefiningOp());
         auto& colManager = getContext()->getLoadedDialect<mlir::tuples::TupleStreamDialect>()->getColumnManager();
         listRef = colManager.createRef(listDef.getName());
         if (!windowOp->hasAttr("needPartition")) 
            rewriter.create<mlir::subop::ANNIncreaseFilteredKOp>(windowOp->getLoc(), adaptor.getRel(), listRef);
      }
      ...
      performWindowOp(windowOp, adaptor.getRel(), rewriter, evaluate, listRef, windowOp->hasAttr("needPartition"));
      return success();
   }
};
...
void RelalgToSubOpLoweringPass::runOnOperation() {
   ...
   patterns.insert<ANNRangeSelectionLowering>(typeConverter, ctxt);
   patterns.insert<ANNTopKLowering>(typeConverter, ctxt);

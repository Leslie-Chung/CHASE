...
class AtLowering : public OpConversionPattern<mlir::dsa::At> {
   ...
   LogicalResult matchAndRewrite(mlir::dsa::At atOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      uint32_t pos = atOp.getPos();
      // if(auto denseVectorType = t.dyn_cast_or_null<mlir::db::DenseVectorType>()) {
      //    pos *= denseVectorType.getSize();
      // }
      auto newAtOp = rewriter.create<mlir::dsa::At>(loc, types, adaptor.getCollection(), pos);
      ...
      if (auto denseVectorType = t.dyn_cast_or_null<mlir::db::DenseVectorType>()) { // support vector
         newAtOp->setAttr("numBytes", rewriter.getI32IntegerAttr(denseVectorType.getSize()));
      }
...
class L2DistanceLowering : public OpConversionPattern<mlir::db::L2Distance> { // support vector
   public:
   using OpConversionPattern<mlir::db::L2Distance>::OpConversionPattern;
   LogicalResult matchAndRewrite(mlir::db::L2Distance l2Distance, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      auto baseLeftType = getBaseType(l2Distance.getLeft().getType()).dyn_cast<mlir::db::DenseVectorType>();
      auto left = l2Distance.getLeft();
      auto right = l2Distance.getRight();
      uint32_t size = baseLeftType.getSize();
      Location loc = l2Distance->getLoc();
      
      Value v1 = left, v2 = right;

      Value vec1 = rewriter.create<mlir::db::CastOp>(loc, typeConverter->convertType(baseLeftType), v1);
      Value vec2 = rewriter.create<mlir::db::CastOp>(loc, typeConverter->convertType(baseLeftType), v2);
      Value res = rewriter.create<arith::ConstantOp>(loc, rewriter.getF32Type(), rewriter.getF32FloatAttr(0));
      auto loopBody = [&](OpBuilder& nestedBuilder, Location nestedLoc, Value iv,
                          ValueRange iterArgs) {
         auto t1 = nestedBuilder.create<mlir::util::LoadOp>(nestedLoc, vec1, iv);
         auto t2 = nestedBuilder.create<mlir::util::LoadOp>(nestedLoc, vec2, iv);
         auto tmp1 = nestedBuilder.create<arith::SubFOp>(nestedLoc, t1, t2);
         auto tmp2 = nestedBuilder.create<arith::MulFOp>(nestedLoc, tmp1, tmp1);
         auto tmpRes = nestedBuilder.create<arith::AddFOp>(nestedLoc, iterArgs[0], tmp2);
         nestedBuilder.create<scf::YieldOp>(nestedLoc, ValueRange({tmpRes}));
      };

      Value lowerBound = rewriter.create<arith::ConstantIndexOp>(loc, 0);
      Value upperBound = rewriter.create<arith::ConstantIndexOp>(loc, size);
      Value step = rewriter.create<arith::ConstantIndexOp>(loc, 1);
      auto forop = rewriter.create<scf::ForOp>(loc, /*lowerBound=*/lowerBound, /*upperBound=*/upperBound, /*step=*/step, /*iterArgs=*/res, loopBody);
      // auto forop = rewriter.create<scf::ForOp>(loc, /*lowerBound=*/0, /*upperBound=*/size, /*step=*/1, /*iterArgs=*/res, loopBody);
      
      rewriter.replaceOp(l2Distance, forop);
      return success();
   }
};

class InnerProductLowering : public OpConversionPattern<mlir::db::InnerProduct> { // support vector
   public:
   using OpConversionPattern<mlir::db::InnerProduct>::OpConversionPattern;
   LogicalResult matchAndRewrite(mlir::db::InnerProduct innerProduct, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      auto baseLeftType = getBaseType(innerProduct.getLeft().getType()).dyn_cast<mlir::db::DenseVectorType>();
      auto left = innerProduct.getLeft();
      auto right = innerProduct.getRight();
      uint32_t size = baseLeftType.getSize();
      Location loc = innerProduct->getLoc();
      ...
      auto loopBody = [&](OpBuilder& nestedBuilder, Location nestedLoc, Value iv,
         ...
         auto t1 = nestedBuilder.create<mlir::util::LoadOp>(nestedLoc, vec1, iv);
         auto t2 = nestedBuilder.create<mlir::util::LoadOp>(nestedLoc, vec2, iv);
         auto tmp2 = nestedBuilder.create<arith::MulFOp>(nestedLoc, t1, t2);
         auto tmpRes = nestedBuilder.create<arith::SubFOp>(nestedLoc, iterArgs[0], tmp2);
         nestedBuilder.create<scf::YieldOp>(nestedLoc, ValueRange({tmpRes}));
      };
      ...
      auto forop = rewriter.create<scf::ForOp>(loc, /*lowerBound=*/lowerBound, /*upperBound=*/upperBound, /*step=*/step, /*iterArgs=*/res, loopBody);
      // auto forop = rewriter.create<scf::ForOp>(loc, /*lowerBound=*/0, /*upperBound=*/size, /*step=*/1, /*iterArgs=*/res, loopBody);
      // auto forop2 = rewriter.create<scf::ForOp>(loc, /*lowerBound=*/lowerBound, /*upperBound=*/upperBound, /*step=*/step, /*iterArgs=*/forop->getResult(0), loopBody);
      // rewriter.replaceOp(innerProduct, forop2);
      rewriter.replaceOp(innerProduct, forop);
      return success();
   }
};
...
class ConstantLowering : public OpConversionPattern<mlir::db::ConstantOp> {
   static std::tuple<arrow::Type::type, uint32_t, uint32_t> convertTypeToArrow(mlir::Type type) {
      ...
      if (isIntegerType(type, 1)) {
         ...
         typeConstant = arrow::Type::type::FIXED_SIZE_LIST;
         param1 = static_cast<uint32_t>(denseVectorType.getSize());
      }
   ...
   LogicalResult matchAndRewrite(mlir::db::ConstantOp constantOp, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      } else if (auto denseArrayAttr = constantOp.getValue().dyn_cast_or_null<DenseF32ArrayAttr>()) { // support vector
         const float* data = (const float*) denseArrayAttr.getRawData().data();
         parseArg = reinterpret_cast<int64_t>(data);
      } else {
         return failure();
      }
      ...
      if (auto intType = stdType.dyn_cast_or_null<IntegerType>()) {
         ...
         uint64_t address = (uint64_t) std::get<int64_t>(parseResult);
         float* data = reinterpret_cast<float*>(address);
         Location loc = constantOp.getLoc();
         auto float32Type = mlir::Float32Type::get(rewriter.getContext());
         auto refType = mlir::util::RefType::get(rewriter.getContext(), float32Type);
         auto size = rewriter.create<arith::ConstantIndexOp>(loc, param1);
         auto alloc = rewriter.create<mlir::util::AllocOp>(loc, refType, size);
         ...
         auto dealloc = rewriter.create<mlir::util::DeAllocOp>(loc, alloc);
         ...
         SmallVector<Value, 8> constantFloats;
         for (auto i : llvm::seq<int64_t>(0, param1)) {
            constantIndices.push_back(rewriter.create<arith::ConstantIndexOp>(loc, i));
            llvm::APFloat tmp(data[i]);
         }
         for (auto i : llvm::seq<int64_t>(0, param1)) {
            llvm::APFloat tmp(data[i]);
            constantFloats.push_back(rewriter.create<arith::ConstantFloatOp>(loc, tmp, float32Type));
         }
         for (uint64_t i = 0; i != param1; ++i) {
            rewriter.create<mlir::util::StoreOp>(loc, constantFloats[i], alloc, constantIndices[i]);
         }

         rewriter.replaceOp(constantOp, alloc);
         return success();
      } else {
         return failure();
      }
...
class CastOpLowering : public OpConversionPattern<mlir::db::CastOp> { 
   ...
   LogicalResult matchAndRewrite(mlir::db::CastOp op, OpAdaptor adaptor, ConversionPatternRewriter& rewriter) const override {
      ...
      if (scalarSourceType == scalarTargetType) { // nullable -> nullable
      ...
      } else if (auto denseVectorSourceType = scalarSourceType.dyn_cast_or_null<db::DenseVectorType>()) { // support vector
         if (auto RefTargetType = scalarTargetType.dyn_cast_or_null<util::RefType>()) {
            rewriter.replaceOp(op, value);
            return success();
         }
      }
...
void DBToStdLoweringPass::runOnOperation() {
   ...
   typeConverter.addConversion([&](::mlir::db::DenseVectorType t) { // support vector
      auto refType = mlir::util::RefType::get(ctxt, mlir::Float32Type::get(ctxt));
   ...
   // target.addDynamicallyLegalDialect<affine::AffineDialect>(opIsWithoutDBTypes); // support vector
   // target.addLegalDialect<affine::AffineDialect>(); // support vector
   ...
   // mlir::populateAffineToStdConversionPatterns(patterns); // support vector
   ...
   patterns.insert<L2DistanceLowering>(typeConverter, ctxt); // support vector
   patterns.insert<InnerProductLowering>(typeConverter, ctxt); // support vector

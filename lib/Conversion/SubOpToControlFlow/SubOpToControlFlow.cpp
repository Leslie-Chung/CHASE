...
#include "runtime-defs/HNSWIndex.h"
...
class SubOpRewriter {
   ...
   void replaceTupleStream(mlir::Value tupleStream, ColumnMapping& mapping) { 
      ...
      // inFlightTupleStreams[tupleStream].inFlightOp->dump();
   }
...
class TableRefGatherOpLowering : public SubOpTupleStreamConsumerConversionPattern<mlir::subop::GatherOp, 2> {
   ...
   LogicalResult matchAndRewrite(mlir::subop::GatherOp gatherOp, OpAdaptor adaptor, SubOpRewriter& rewriter, ColumnMapping& mapping) const override {
      ...
      // @scan::@ref({type = !subop.table_entry_ref<[category$0 : !db.string, id$0 : i32, pname$0 : !db.string, price$0 : !db.nullable<f32>, vec$0 : !db.nullable<!db.densevector<4>>]>})
      auto currRow = mapping.resolve(gatherOp.getRef()); 
      for (size_t i = 0; i < columns.getTypes().size(); i++) {
         auto memberName = columns.getNames()[i].cast<mlir::StringAttr>().str(); // vec$0
         if (gatherOp.getMapping().contains(memberName)) { 
            // mapping-> {id$0 => @products::@id({type = i32}), price$0 => @products::@price({type = !db.nullable<f32>}), vec$0 => @products::@vec({type = !db.nullable<!db.densevector<4>>})}
            auto columnDefAttr = gatherOp.getMapping().get(memberName).cast<mlir::tuples::ColumnDefAttr>(); // @products::@vec
            auto type = columnDefAttr.getColumn().type; // !db.nullable<!db.densevector<4>>
            ...
            if (type.isa<mlir::db::NullableType>()) {
               ...
               mapping.define(columnDefAttr, atOp.getVal()); // @products::@vec -> atopValue
            }
         }
      }
...
class GetExternalHNSWIndexLowering : public SubOpConversionPattern<mlir::subop::GetExternalOp> { // support vector
   public:
   using SubOpConversionPattern<mlir::subop::GetExternalOp>::SubOpConversionPattern;

   LogicalResult matchAndRewrite(mlir::subop::GetExternalOp op, OpAdaptor adaptor, SubOpRewriter& rewriter) const override {
      if (!op.getType().isa<mlir::subop::ExternalHNSWIndexType>()) return failure();
      mlir::Value description = rewriter.create<mlir::util::CreateConstVarLen>(op->getLoc(), mlir::util::VarLen32Type::get(rewriter.getContext()), op.getDescrAttr());

      rewriter.replaceOp(op, rt::RelationHelper::getHNSWIndex(rewriter, op->getLoc())({getExecutionContext(rewriter, op), description})[0]);
      return mlir::success();
   }
};
...
class CreateTableLowering : public SubOpConversionPattern<mlir::subop::CreateResultTableOp> { 
   ...
   std::string arrowDescrFromType(mlir::Type type) const {
      ...
      } else if (auto denseVectorType = type.dyn_cast_or_null<mlir::db::DenseVectorType>()) { // support vector
         return "vector[" + std::to_string(static_cast<uint32_t>(denseVectorType.getSize())) + "]";
      }
      return "";
   }
   ...
   LogicalResult matchAndRewrite(mlir::subop::ScanRefsOp scanOp, OpAdaptor adaptor, SubOpRewriter& rewriter) const override {
      ...
      rewriter.atStartOf(funcBody, [&](SubOpRewriter& rewriter) {
         ...
         rewriter.atStartOf(block2, [&](SubOpRewriter& rewriter) {
            // @scan::@ref({type = !subop.table_entry_ref<[category$0 : !db.string, id$0 : i32, pname$0 : !db.string, price$0 : !db.nullable<f32>, vec$0 : !db.nullable<!db.densevector<4>>]>}) -> %arg2 (dsa.for %arg2 in %5 里对应的%arg2)
      ...
      rt::DataSourceIteration::end(rewriter, scanOp->getLoc())({iterator});
      return success();
   }
};
...
class ScanExternalHashIndexListLowering : public SubOpConversionPattern<mlir::subop::ScanListOp> {
   ...
   LogicalResult matchAndRewrite(mlir::subop::ScanListOp scanOp, OpAdaptor adaptor, SubOpRewriter& rewriter) const override {
      ...
      auto baseTypes = [](mlir::TypeRange arr) { // keysType + valuesType
         std::vector<Type> res;
         for (auto x : arr) { res.push_back(getBaseType(x)); }
         return res;
      };
...
class ANNScanExternalHNSWIndexListLowering : public SubOpConversionPattern<mlir::subop::ANNScanOp> {
   public:
   using SubOpConversionPattern<mlir::subop::ANNScanOp>::SubOpConversionPattern;

   LogicalResult matchAndRewrite(mlir::subop::ANNScanOp scanOp, OpAdaptor adaptor, SubOpRewriter& rewriter) const override {
      auto listType = scanOp.getList().getType().dyn_cast_or_null<mlir::subop::ListType>();
      if (!listType) return mlir::failure();

      mlir::subop::ExternalHNSWIndexType externalHNSWIndexType;
      if (auto lookupRefType = listType.getT().dyn_cast_or_null<mlir::subop::LookupEntryRefType>()) {
         if (!(externalHNSWIndexType = lookupRefType.getState().dyn_cast_or_null<mlir::subop::ExternalHNSWIndexType>())) {
            return mlir::failure();
         };
      } else if (auto entryRefType = listType.getT().dyn_cast_or_null<mlir::subop::ExternalHNSWIndexEntryRefType>()) {
         externalHNSWIndexType = entryRefType.getExternalHNSWIndex();
      } else {
         return mlir::failure();
      }
      ...
      // Get correct types
      auto tupleType = mlir::TupleType::get(ctxt, unpackTypes(externalHNSWIndexType.getMembers().getTypes()));
      auto baseTypes = [](mlir::TypeRange arr) { // keysType + valuesType
         std::vector<Type> res;
         for (auto x : arr) { res.push_back(getBaseType(x)); }
         return res;
      ...
      // rt::HNSWIndexIteration::doTop1(rewriter, loc)({adaptor.getList()});
      // Check if iterator contains another value
      rewriter.atStartOf(conditionBlock, [&](SubOpRewriter& rewriter) {
         mlir::Value list = conditionBlock->getArgument(0);
         mlir::Value cont = rt::HNSWIndexIteration::hasNext(rewriter, loc)({list})[0];
         rewriter.create<scf::ConditionOp>(loc, cont, ValueRange({list}));
      });
      // mlir::Value range = rewriter.create<mlir::db::ConstantOp>(loc, mlir::Float32Type::get(ctxt), mlir::FloatAttr::get(mlir::Float32Type::get(ctxt), 0));
      // Load record batch from iterator
      rewriter.atStartOf(bodyBlock, [&](SubOpRewriter& rewriter) {
         mlir::Value list = bodyBlock->getArgument(0);
         mlir::Value recordBatchPointer;
         ...
         rt::HNSWIndexIteration::consumeRecordBatch(rewriter, loc)({list, recordBatchPointer});
         mlir::Value recordBatch = rewriter.create<mlir::util::LoadOp>(loc, recordBatchPointer, mlir::Value());
         // load tuple from record batch
         auto forOp2 = rewriter.create<mlir::dsa::ForOp>(scanOp->getLoc(), mlir::TypeRange{}, recordBatch, mlir::ValueRange{});
         {
            ...
            rewriter.atStartOf(block2, [&](SubOpRewriter& rewriter) {
               mapping.define(scanOp.getElem(), block2->getArgument(0));
               mapping.define(scanOp.getComputedCol(), rt::HNSWIndexIteration::getCurrentDistance(rewriter, loc)({list})[0]);
               // mapping.define(scanOp.getComputedCol(), range);
               rewriter.replaceTupleStream(scanOp, mapping);
               rewriter.create<mlir::dsa::YieldOp>(scanOp->getLoc());
            });
         }
      ...
      // Close iterator
      rt::HNSWIndexIteration::close(rewriter, loc)({adaptor.getList()});
      return success();
   }
};


class ANNLookupAndScanLowering : public SubOpConversionPattern<mlir::subop::ANNLookupAndScanOp> {
   public:
   using SubOpConversionPattern<mlir::subop::ANNLookupAndScanOp>::SubOpConversionPattern;

   LogicalResult matchAndRewrite(mlir::subop::ANNLookupAndScanOp lookupAndScanOp, OpAdaptor adaptor, SubOpRewriter& rewriter) const override {
      auto externalHNSWIndexType = lookupAndScanOp.getState().getType().dyn_cast_or_null<mlir::subop::ExternalHNSWIndexType>();
      if (!externalHNSWIndexType) {
         return mlir::failure();
      }
      auto loc = lookupAndScanOp->getLoc();
      auto* ctxt = rewriter.getContext();
      ...
      auto queryAttr = lookupAndScanOp.getQueryAttr();
      mlir::Value query = rewriter.create<mlir::db::ConstantOp>(loc, mlir::db::DenseVectorType::get(ctxt, queryAttr.size()), queryAttr);
      mlir::Value q = rewriter.create<mlir::db::CastOp>(loc, mlir::util::RefType::get(ctxt, mlir::Float32Type::get(ctxt)), query);
      mlir::ArrayAttr parameters = lookupAndScanOp.getParametersAttr();

      mlir::Value k = rewriter.create<mlir::db::ConstantOp>(loc, mlir::IntegerType::get(ctxt, 32), parameters[0]);
      mlir::Value range = rewriter.create<mlir::db::ConstantOp>(loc, mlir::Float32Type::get(ctxt), parameters[1]);
      mlir::Value iterator = rt::HNSWIndexAccess::beginScan(rewriter, loc)({adaptor.getState(), q, k, range})[0];


      auto convertedTupleType = mlir::TupleType::get(ctxt, baseTypes(typeRange));
      auto iteratorType = typeConverter->convertType(externalHNSWIndexType);
      auto recordBatchType = mlir::dsa::RecordBatchType::get(ctxt, convertedTupleType);

      // Create while loop to extract all chained values from hash table
      auto whileOp = rewriter.create<mlir::scf::WhileOp>(loc, iteratorType, iterator);
      Block* conditionBlock = new Block;
      Block* bodyBlock = new Block;
      whileOp.getBefore().push_back(conditionBlock);
      whileOp.getAfter().push_back(bodyBlock);

      conditionBlock->addArgument(iteratorType, loc);
      bodyBlock->addArgument(iteratorType, loc);
      ColumnMapping mapping;
      mapping.define(lookupAndScanOp.getRef(), iterator);
      
      // Check if iterator contains another value
      rewriter.atStartOf(conditionBlock, [&](SubOpRewriter& rewriter) {
         mlir::Value iterator = conditionBlock->getArgument(0);
         mlir::Value cont = rt::HNSWIndexIteration::hasNext(rewriter, loc)({iterator})[0];
         rewriter.create<scf::ConditionOp>(loc, cont, ValueRange({iterator}));
      });
      ...
      rewriter.atStartOf(bodyBlock, [&](SubOpRewriter& rewriter) {
         mlir::Value list = bodyBlock->getArgument(0);
         mlir::Value recordBatchPointer;
         rewriter.atStartOf(&lookupAndScanOp->getParentOfType<mlir::func::FuncOp>().getBody().front(), [&](SubOpRewriter& rewriter) {
            recordBatchPointer = rewriter.create<mlir::util::AllocaOp>(loc, mlir::util::RefType::get(rewriter.getContext(), recordBatchType), mlir::Value());
         });
         rt::HNSWIndexIteration::consumeRecordBatch(rewriter, loc)({list, recordBatchPointer});
         mlir::Value recordBatch = rewriter.create<mlir::util::LoadOp>(loc, recordBatchPointer, mlir::Value());
         // load tuple from record batch
         auto forOp2 = rewriter.create<mlir::dsa::ForOp>(lookupAndScanOp->getLoc(), mlir::TypeRange{}, recordBatch, mlir::ValueRange{});
         {
            mlir::Block* block2 = new mlir::Block;
            block2->addArgument(recordBatchType.getElementType(), lookupAndScanOp->getLoc()); 
            forOp2.getBodyRegion().push_back(block2);
            rewriter.atStartOf(block2, [&](SubOpRewriter& rewriter) {
               mapping.define(lookupAndScanOp.getElem(), block2->getArgument(0));
               
               mapping.define(lookupAndScanOp.getComputedCol(), rt::HNSWIndexIteration::getCurrentDistance(rewriter, loc)({list})[0]);
               // mapping.define(lookupAndScanOp.getComputedCol(), range);
               rewriter.replaceTupleStream(lookupAndScanOp, mapping);
               rewriter.create<mlir::dsa::YieldOp>(lookupAndScanOp->getLoc());
            });
         }
         rewriter.create<scf::YieldOp>(loc, list);
      });
      ...
      // Close iterator
      rt::HNSWIndexIteration::close(rewriter, loc)({iterator});
      return success();
   }
};
   ...
   LogicalResult matchAndRewrite(mlir::subop::MapOp mapOp, OpAdaptor adaptor, SubOpRewriter& rewriter, ColumnMapping& mapping) const override {
      {
         ...
         auto* terminator = source->getTerminator(); // == mlir::tuples::ReturnOp
...
class ANNIncreaseFilteredKOpLowering : public SubOpTupleStreamConsumerConversionPattern<mlir::subop::ANNIncreaseFilteredKOp> {
   public:
   using SubOpTupleStreamConsumerConversionPattern<mlir::subop::ANNIncreaseFilteredKOp>::SubOpTupleStreamConsumerConversionPattern;

   LogicalResult matchAndRewrite(mlir::subop::ANNIncreaseFilteredKOp annIncreaseFilteredKOp, OpAdaptor adaptor, SubOpRewriter& rewriter, ColumnMapping& mapping) const override {
      // annIncreaseFilteredKOp.getStream().dump();
      // annIncreaseFilteredKOp->getParentOp()->dump();
      mlir::Value iterator = mapping.resolve(annIncreaseFilteredKOp.getIteratorRef());
      rt::HNSWIndexIteration::addFilteredK(rewriter, annIncreaseFilteredKOp.getLoc())({iterator});
      rewriter.eraseOp(annIncreaseFilteredKOp);
      return success();
   }
};

class ANNSetPartitionOpLowering : public SubOpTupleStreamConsumerConversionPattern<mlir::subop::ANNSetPartitionOp> {
   public:
   using SubOpTupleStreamConsumerConversionPattern<mlir::subop::ANNSetPartitionOp>::SubOpTupleStreamConsumerConversionPattern;

   LogicalResult matchAndRewrite(mlir::subop::ANNSetPartitionOp annSetPartitionOp, OpAdaptor adaptor, SubOpRewriter& rewriter, ColumnMapping& mapping) const override {
      // annIncreaseFilteredKOp.getStream().dump();
      // annIncreaseFilteredKOp->getParentOp()->dump();
      mlir::Value iterator = mapping.resolve(annSetPartitionOp.getIteratorRef());
      mlir::Value key = mapping.resolve(annSetPartitionOp.getValueRef());
      rt::HNSWIndexIteration::setCategory(rewriter, annSetPartitionOp.getLoc())({iterator, key});
      rewriter.eraseOp(annSetPartitionOp);
      return success();
   }
};
...
class LookupHashMapLowering : public SubOpTupleStreamConsumerConversionPattern<mlir::subop::LookupOrInsertOp> {
   ...
   LogicalResult matchAndRewrite(mlir::subop::LookupOrInsertOp lookupOp, OpAdaptor adaptor, SubOpRewriter& rewriter, ColumnMapping& mapping) const override {
      ...
      Value kvAddress = rewriter.create<util::TupleElementPtrOp>(loc, kvPtrType, currEntryPtr, 2);
      // define value ptr
      mapping.define(lookupOp.getRef(), rewriter.create<mlir::util::TupleElementPtrOp>(lookupOp->getLoc(), mlir::util::RefType::get(getContext(), kvType.getType(1)), kvAddress, 1)); // value
      rewriter.replaceTupleStream(lookupOp, mapping);
      return mlir::success();
   }
};
...
class LookupExternalHNSWIndexLowering : public SubOpTupleStreamConsumerConversionPattern<mlir::subop::LookupOp> {  // support vector
   public:
   using SubOpTupleStreamConsumerConversionPattern<mlir::subop::LookupOp>::SubOpTupleStreamConsumerConversionPattern;
   LogicalResult matchAndRewrite(mlir::subop::LookupOp lookupOp, OpAdaptor adaptor, SubOpRewriter& rewriter, ColumnMapping& mapping) const override {
      if (!lookupOp.getState().getType().isa<mlir::subop::ExternalHNSWIndexType>()) return failure();

      auto loc = lookupOp->getLoc();
      auto *ctxt = rewriter.getContext();
      // keys == colname dim distancespace range_constant k_const
      auto hashLeft = lookupOp.getKeys();
      auto query = mapping.resolve(hashLeft[0].cast<mlir::tuples::ColumnRefAttr>());
      mlir::Value q = rewriter.create<mlir::db::CastOp>(loc, mlir::util::RefType::get(ctxt, mlir::Float32Type::get(ctxt)), query);
      // mlir::Value q = rewriter.create<mlir::UnrealizedConversionCastOp>(loc, mlir::util::RefType::get(ctxt, mlir::Float32Type::get(ctxt)), query)->getResult(0);

      mlir::Value k = rewriter.create<mlir::db::ConstantOp>(loc, mlir::IntegerType::get(ctxt, 32), hashLeft[4]);
      mlir::Value range = rewriter.create<mlir::db::ConstantOp>(loc, mlir::Float32Type::get(ctxt), hashLeft[3]);      
      mlir::Value list = rt::HNSWIndexAccess::beginScan(rewriter, loc)({adaptor.getState(), q, k, range})[0]; 
...
class ExternalHNSWIndexRefGatherOpLowering : public SubOpTupleStreamConsumerConversionPattern<mlir::subop::GatherOp, 2> {
   public:
   using SubOpTupleStreamConsumerConversionPattern<mlir::subop::GatherOp, 2>::SubOpTupleStreamConsumerConversionPattern;

   LogicalResult matchAndRewrite(mlir::subop::GatherOp gatherOp, OpAdaptor adaptor, SubOpRewriter& rewriter, ColumnMapping& mapping) const override {
      auto refType = gatherOp.getRef().getColumn().type;
      if (!refType.isa<mlir::subop::ExternalHNSWIndexEntryRefType>()) { return failure(); }
      auto columns = refType.cast<mlir::subop::ExternalHNSWIndexEntryRefType>().getMembers();
      auto currRow = mapping.resolve(gatherOp.getRef()); 
...
void SubOpToControlFlowLoweringPass::runOnOperation() {
   ...
   typeConverter.addConversion([&](mlir::subop::ExternalHNSWIndexType t) -> Type {
      return mlir::util::RefType::get(t.getContext(), mlir::IntegerType::get(ctxt, 8));
   });
   typeConverter.addConversion([&](mlir::subop::ListType t) -> Type {
      if (auto lookupEntryRefType = t.getT().dyn_cast_or_null<mlir::subop::LookupEntryRefType>()) {
         ...
         if (auto externalHashIndexRefType = t.getT().dyn_cast_or_null<mlir::subop::ExternalHNSWIndexType>()) {
            return mlir::util::RefType::get(t.getContext(), mlir::IntegerType::get(ctxt, 8));
         }
      ...
      if (auto externalHNSWIndexRefType = t.getT().dyn_cast_or_null<mlir::subop::ExternalHNSWIndexEntryRefType>()) {
         return mlir::util::RefType::get(t.getContext(), mlir::IntegerType::get(ctxt, 8));
      }
      return mlir::Type();
   });
   ...
   // rewriter.insertPattern<ANNRangeLowering>(typeConverter, ctxt);
   ...
   rewriter.insertPattern<GetExternalHNSWIndexLowering>(typeConverter, ctxt); // support vector
   //ResultTable
   ...
   // ExternalHNSWIndex
   rewriter.insertPattern<ANNLookupAndScanLowering>(typeConverter, ctxt); // support vector
   rewriter.insertPattern<ExternalHNSWIndexRefGatherOpLowering>(typeConverter, ctxt);  // support vector
   rewriter.insertPattern<ANNIncreaseFilteredKOpLowering>(typeConverter, ctxt);  // support vector
   rewriter.insertPattern<ANNSetPartitionOpLowering>(typeConverter, ctxt);  // support vector
   rewriter.insertPattern<ANNScanExternalHNSWIndexListLowering>(typeConverter, ctxt);
   rewriter.insertPattern<LookupExternalHNSWIndexLowering>(typeConverter, ctxt);

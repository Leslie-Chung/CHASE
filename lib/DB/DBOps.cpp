...
uint8_t mlir::db::L2Distance::getDistanceSpace() { return 1; }
uint8_t mlir::db::InnerProduct::getDistanceSpace() { return 2; }
...
OpFoldResult mlir::db::ConstantOp::fold(mlir::db::ConstantOp::FoldAdaptor adaptor) {
   ...
   } else if (auto denseVectorAttr = getValue().dyn_cast_or_null<DenseF32ArrayAttr>()) { // support vector
      return getValue();
   } else {
      // puts("error");
      return {};
...
LogicalResult mlir::db::L2Distance::verify() {
   auto type1 = getBaseType(getLeft().getType()).dyn_cast<mlir::db::DenseVectorType>();
   auto type2 = getBaseType(getRight().getType()).dyn_cast<mlir::db::DenseVectorType>();
   if (type1.getSize() != type2.getSize()) {
      return failure();
   }
   return success();
}
LogicalResult mlir::db::InnerProduct::verify() {
   auto type1 = getBaseType(getLeft().getType()).dyn_cast<mlir::db::DenseVectorType>();
   auto type2 = getBaseType(getRight().getType()).dyn_cast<mlir::db::DenseVectorType>();
   if (type1.getSize() != type2.getSize()) {
      return failure();

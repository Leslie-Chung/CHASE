...
const std::vector<std::string>& runtime::TableMetaData::getVectorColumn() const {
   std::vector<std::string>* col = new std::vector<std::string>;
   for (const auto& column : columns) {
      if (column.second->getColumnType().base == "vector") {
         col->push_back(column.first);
      }
   }
   return *col;
}

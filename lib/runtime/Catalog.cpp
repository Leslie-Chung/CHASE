...
class EmptyCatalog : public Catalog {
   ...
   std::string getDBDirectory() override{
      throw std::runtime_error("can not getDBDirectory");
   }
};
...
std::string LocalCatalog::getDBDirectory(){
   throw std::runtime_error("can not getDBDirectory");
}
...
void DBCatalog::addTable(std::string tableName, std::shared_ptr<TableMetaData> mD) {
   ...
   relations[tableName]->setPersist(true);
}

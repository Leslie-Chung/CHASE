...
std::vector<std::shared_ptr<arrow::RecordBatch>> toRecordBatches(std::shared_ptr<arrow::Table> table) {
   ...
   int64_t chuckSize = table->num_rows() / 8;
   if (chuckSize == 0) {
      chuckSize = 100000;
   }
   reader.set_chunksize(chuckSize);
...
std::shared_ptr<arrow::DataType> createDataType(const runtime::ColumnType& columnType) {
   ...
   if (columnType.base == "vector") return arrow::fixed_size_list(arrow::float32(), asInt(columnType.modifiers.at(0))); // vector length
   #else
   if (columnType.base == "vector") return arrow::fixed_size_binary(asInt(columnType.modifiers.at(0)) * sizeof(float)); // element_num * sizeof(float) support vector
   #endif
...
class DBRelation : public Relation {
      ...
      for (auto index : metaData->getIndices()) {
         switch (index->type) {
            case Index::Type::HASH:
               ...
               break;
            case Index::Type::HNSW:
               indices.insert({index->name, Index::createHNSWIndex(*index, *this, dbDir)}); 
               break;
         }  
      }
   ...
   void setIndex(const std::string name, std::shared_ptr<Index> index) override {
      if (indices.contains(name)) {
         throw std::runtime_error("index exists");
         return;
      }

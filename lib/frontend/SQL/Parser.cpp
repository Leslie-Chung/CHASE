...
frontend::sql::ExpressionType frontend::sql::stringToExpressionType(const std::string& parserStr) {
   ...
   return llvm::StringSwitch<ExpressionType>(str)
      ...
      .Case("<->", ExpressionType::OPERATOR_L2DISTANCE) // support vector
      .Case("<#>", ExpressionType::OPERATOR_INNERPRODUCT)
            ...
            case T_A_Const: {
               ...
               switch (constVal.type_) {
                  ...
                  case T_String: {
                     ...
                     if (stringVal.front() == '[' && stringVal.back() == ']') { // support vector
                        std::vector<float> numbers;
                        stringVal = stringVal.substr(1, stringVal.size() - 2);
                        ...
                        t = mlir::db::DenseVectorType::get(builder.getContext(), numbers.size());
                        value = builder.getDenseF32ArrayAttr(numbers);
                        break;
                     }
...
mlir::Value frontend::sql::Parser::translateBinaryExpression(mlir::OpBuilder& builder, frontend::sql::ExpressionType opType, mlir::Value left, mlir::Value right) {
   ...
   switch (opType) {
      ...
      case ExpressionType::OPERATOR_L2DISTANCE: { // support vector
         return builder.create<mlir::db::L2Distance>(builder.getUnknownLoc(), SQLTypeInference::toCommonBaseTypes(builder, {left, right}));
      }
      case ExpressionType::OPERATOR_INNERPRODUCT: { // support vector
         return builder.create<mlir::db::InnerProduct>(builder.getUnknownLoc(), SQLTypeInference::toCommonBaseTypes(builder, {left, right}));
      }
      default:
...
mlir::Value frontend::sql::Parser::translateFromClause(mlir::OpBuilder& builder, SelectStmt* stmt, TranslationContext& context, TranslationContext::ResolverScope& scope) {
   ...
   if (!fromClause) { return mlir::Value(); }; // mlir::Value represents an instance of an SSA value in the MLIR system, representing a computable value that has a type and a set of users.
   mlir::Value last;
...
mlir::Value frontend::sql::Parser::translateExpression(mlir::OpBuilder& builder, Node* node, TranslationContext& context, bool ignoreNull) {
   ...
   switch (node->type) {
         ...
         switch (constVal.type_) {
            ...
            case T_String: {
               ...
               if (stringVal.front() == '[' && stringVal.back() == ']') { // support vector
                  ...
                  mlir::Type denseVectorType = mlir::db::DenseVectorType::get(builder.getContext(), numbers.size());
                  return builder.create<mlir::db::ConstantOp>(loc, denseVectorType, builder.getDenseF32ArrayAttr(numbers));
               }
      ...
      case T_CaseExpr: { // CASE 0 WHEN x THEN y ELSE z END
...
void frontend::sql::Parser::translateIndexStatement(mlir::OpBuilder& builder, IndexStmt* statement) {
   std::string accessMethod = statement->access_method_ != nullptr ? statement->access_method_ : "";
   if (accessMethod.compare("hnsw") == 0) {
      RangeVar* relation = statement->relation_;
      std::string relationName = relation->relname_ != nullptr ? relation->relname_ : "";
      auto relationNameValue = createStringValue(builder, relationName);
      List* indexParams = statement->index_params_;
      std::string distanceFunctionType = reinterpret_cast<value*>(reinterpret_cast<IndexElem*>(indexParams->head->data.ptr_value)->opclass_->head->data.ptr_value)->val_.str_;
      auto distanceFunctionTypeValue = createStringValue(builder, distanceFunctionType);
      mlir::arith::ConstantIntOp efconstructionValue = builder.create<mlir::arith::ConstantIntOp>(builder.getUnknownLoc(), 200, builder.getI32Type());
      mlir::arith::ConstantIntOp mValue = builder.create<mlir::arith::ConstantIntOp>(builder.getUnknownLoc(), 16, builder.getI32Type());
      mlir::arith::ConstantIntOp maxelementsValue = builder.create<mlir::arith::ConstantIntOp>(builder.getUnknownLoc(), 1000000, builder.getI32Type());
      for (auto* optionCell = statement->options_->head; optionCell != nullptr; optionCell = optionCell->next) {
         auto optionElem = reinterpret_cast<DefElem*>(optionCell->data.ptr_value);
         std::string optionName = optionElem->defname_;
         if (optionName == "m") {
            size_t m = reinterpret_cast<value*>(optionElem->arg_)->val_.ival_;
            mValue = builder.create<mlir::arith::ConstantIntOp>(builder.getUnknownLoc(), m, builder.getI32Type());
         }
         if (optionName == "ef_construction") {
            size_t efconstruction = reinterpret_cast<value*>(optionElem->arg_)->val_.ival_;
            efconstructionValue = builder.create<mlir::arith::ConstantIntOp>(builder.getUnknownLoc(), efconstruction, builder.getI32Type());
         }
         if (optionName == "max_elements") {
            size_t maxelements = reinterpret_cast<value*>(optionElem->arg_)->val_.ival_;
            maxelementsValue = builder.create<mlir::arith::ConstantIntOp>(builder.getUnknownLoc(), maxelements, builder.getI32Type());
         }
      }
      rt::RelationHelper::createHNSWIndex(builder, builder.getUnknownLoc())(mlir::ValueRange{getExecutionContextValue(builder), mValue, efconstructionValue, maxelementsValue, distanceFunctionTypeValue, relationNameValue});
   }
   else if (accessMethod.compare("hash") == 0) {
      RangeVar* relation = statement->relation_;
      std::string relationName = relation->relname_ != nullptr ? relation->relname_ : "";
      auto relationNameValue = createStringValue(builder, relationName);
      std::string indexName(statement->idxname_ != nullptr ? statement->idxname_ : "");
      auto indexNameValue = createStringValue(builder, indexName);
      std::string columnsDescription = "";
      List* indexParams = statement->index_params_;
      for (auto* indexParam = indexParams->head; indexParam != nullptr; indexParam = indexParam->next) {
         auto indexElem = reinterpret_cast<IndexElem*>(indexParam->data.ptr_value);
         std::string columnName = indexElem->name_ != nullptr ? indexElem->name_ : "";
         auto columnNameValue = createStringValue(builder, columnName);
         columnsDescription += columnName + ",";
      }
      auto columns = createStringValue(builder, columnsDescription);
      rt::RelationHelper::createHashIndex(builder, builder.getUnknownLoc())(mlir::ValueRange{getExecutionContextValue(builder), relationNameValue, indexNameValue, columns});
   }
   else {
...
std::optional<mlir::Value> frontend::sql::Parser::translate(mlir::OpBuilder& builder) {
   if (result.tree && result.tree->length == 1) {
      ...
      switch (statement->type) {
         case T_VariableSetStmt: { // change a run-time parameter
         ...
         case T_CopyStmt: { // copy data between a file and a table
            auto* copyStatement = reinterpret_cast<CopyStmt*>(statement);
            translateCopyStatement(builder, copyStatement);
            break;
         }
         case T_IndexStmt: {
            translateIndexStatement(builder, reinterpret_cast<IndexStmt*>(statement));
            break;
         }
         case T_SelectStmt: {
...
Node* frontend::sql::Parser::analyzeTargetExpression(Node* node, frontend::sql::ReplaceState& replaceState) {
   ...
   switch (node->type) {
      ...
      case T_GroupingFunc: { // Grouping(XX,XX)
         ...
         if (auto* window = reinterpret_cast<WindowDef*>(funcNode->over_)) {
            ...
            if (funcName == "sum" || funcName == "avg" || funcName == "min" || funcName == "max" || funcName == "count" || funcName == "stddev_samp") {
               ...
               if (funcNode->agg_star_) {
                  ...
                  if (exprNode->type != T_ColumnRef) { 
                     auto* beforeFakeNode = createFakeNode("", exprNode); 
                     replaceState.evalBeforeAggr.insert({beforeFakeNode, exprNode}); // FakeNode*, Node*
                     aggrExpr = beforeFakeNode;
                  } else {
                     aggrExpr = exprNode;
                  }
               }
               replaceState.aggrs.insert({fakeNode, {funcName, aggrExpr, funcNode->agg_distinct_}}); // FakeNode*, std::tuple<std::string, Node*, bool>
               return fakeNode;
            }
            return node;
         }
...
mlir::Type frontend::sql::Parser::createBaseTypeFromColumnType(mlir::MLIRContext* context, const runtime::ColumnType& colType) {
   ...
   if (colType.base == "vector") return mlir::db::DenseVectorType::get(context, asInt(colType.modifiers.at(0)));
   assert(false);
   return mlir::Type();
}

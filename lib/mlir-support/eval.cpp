...
std::unique_ptr<expr> createLiteral(std::variant<int64_t, double, std::string> parsed, std::tuple<arrow::Type::type, uint32_t, uint32_t> t) {
   ...
   switch (type) {
      ...
      case arrow::Type::type::FIXED_SIZE_LIST: { // support vector
         uint64_t size = tp1 * sizeof(float);
         std::shared_ptr<arrow::Buffer> bytes = arrow::AllocateResizableBuffer(size).ValueOrDie();
         uint64_t address = (uint64_t)std::get<int64_t>(parsed);
         float *data = reinterpret_cast<float*>(address);
         memcpy(bytes->mutable_data(), data, size);
         return pack(arrow::compute::literal(std::make_shared<arrow::FixedSizeBinaryScalar>(bytes, arrow::fixed_size_binary(size))));
      }

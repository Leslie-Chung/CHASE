                              ...
                              IndexInfo *indexInfo)
{
    ...
    try
    {
        ...
        builder.ConstructInternalBuilder(DistanceMethod::InnerProduct);
        builder.LoadTuples();
...
IndexScanDesc pase_hnsw_begin_scan(Relation index, int nkeys, int norderbys)
{
    ...
    elog(WARNING, "LoadIndex() took %f ms to execute \n", elapsed_time);
    return scan;
}
    ...
    if (scanState->first)
    {
       ...
       std::string path = std::string(DataDir) + std::string("/") +
        ...
        std::priority_queue<std::pair<float, std::uint64_t>> max_result = 
            PASE_HNSWIndexScan::BeginScan((char *)scanState->workSpace->array.data(),path, 49);
        // printf("pase_result_queue_length = %d\n", max_result.size());
